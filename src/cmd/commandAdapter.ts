...
type CmdRawResult<K extends CommandKey> =
  AwaitedRet<CommandsType[K]> extends {
    ...
    ? Result<SpectaData<AwaitedRet<CommandsType[K]>>, SpectaError<AwaitedRet<CommandsType[K]>>>
...
const crabProxy = new Proxy(
  {} as {
    [K in CommandKey]: (...args: Parameters<CommandsType[K]>) => Promise<CmdRawResult<K>>;
  },
  {
    get(target, prop: PropertyKey, receiver) {
      ...
      return async (...args: unknown[]) => {
        ...
        const value: Raw = await (cmd as (...a: unknown[]) => Promise<Raw>)(...args);
